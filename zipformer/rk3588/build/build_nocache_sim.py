...
NOCACHE_SIM_ONNX = f'{BASE}/encoder-epoch-99-avg-1-cumfix-nocache-sim.onnx'
CUMFIX_ONNX = f'{BASE}/encoder-epoch-99-avg-1-cumfix.onnx'
DST = f'{RKNN_DIR}/encoder-int8-cumfix-nocache-sim.rknn'
...
def save_calib_npy(calib_data, out_dir='/tmp/calib_nocache_sim'):
...
if __name__ == '__main__':
    ...
    print(f"Loading: {NOCACHE_SIM_ONNX}")
    ret = rknn.load_onnx(model=NOCACHE_SIM_ONNX, inputs=INPUT_NAMES, input_size_list=INPUT_SHAPES)
    if ret != 0:
        print(f"Failed: {ret}, trying without explicit inputs...")
        ret = rknn.load_onnx(model=NOCACHE_SIM_ONNX)
        if ret != 0:
            print(f"Failed again: {ret}")
            sys.exit(1)
    print("Building...")
