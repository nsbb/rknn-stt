"""
Try different RKNN build options on nocache model.
Test: optimization levels, quantization types, etc.
"""
...
def save_calib_npy(calib_data, out_dir='/tmp/calib_variants'):
...
VARIANTS = [
    {
        'name': 'opt2',
        'config': dict(target_platform='rk3588', remove_reshape=True,
                       quantized_dtype='asymmetric_quantized-8', optimization_level=2),
    },
    {
        'name': 'opt1',
        'config': dict(target_platform='rk3588', remove_reshape=True,
                       quantized_dtype='asymmetric_quantized-8', optimization_level=1),
    },
    {
        'name': 'dfp8',
        'config': dict(target_platform='rk3588', remove_reshape=True,
                       quantized_dtype='dynamic_fixed_point-8', optimization_level=3),
    },
    {
        'name': 'dfp16',
        'config': dict(target_platform='rk3588', remove_reshape=True,
                       quantized_dtype='dynamic_fixed_point-16', optimization_level=3),
    },
]
...
if __name__ == '__main__':
    ...
    for v in variants:
        name = v['name']
        dst = f'{RKNN_DIR}/encoder-int8-cumfix-nocache-{name}.rknn'
        print(f"\n=== Building {name} ===")
        print(f"Config: {v['config']}")

        rknn = RKNN(verbose=False)
        rknn.config(**v['config'])
