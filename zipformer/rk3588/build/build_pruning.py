"""
시도 10: model_pruning + sparse_infer + compress_weight 조합 테스트.
model_pruning: RKNN 컴파일러가 불필요한 노드를 제거 → 레이어 수 감소 가능.
sparse_infer: 희소 가중치 추론 최적화 → 연산량 감소 가능.
compress_weight: 가중치 압축 → 메모리 전송 감소 가능.
"""
import sys, os, time
...
VARIANTS = [
    ('prune', dict(model_pruning=True)),
    ('sparse', dict(sparse_infer=True)),
    ('compress', dict(compress_weight=True)),
    ('prune-sparse', dict(model_pruning=True, sparse_infer=True)),
    ('all', dict(model_pruning=True, sparse_infer=True, compress_weight=True)),
]
