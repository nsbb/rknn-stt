"""
INT8 cumfix encoder with op_target: Reshape/Transpose → CPU.
Perf analysis showed Reshape(8.2ms) + Transpose(6.7ms) = 14.9ms on NPU.
Moving to CPU should reduce to near-zero (layout ops are metadata only on CPU).
"""
import numpy as np, os, sys, glob, onnx
...
def save_calib_npy(calib_data, out_dir='/tmp/calib_enc_optarget'):
...
def get_op_target():
    """Get Reshape + Transpose node names → CPU."""
    m = onnx.load(CUMFIX_ONNX)
    target = {}
    for n in m.graph.node:
        if n.op_type in ('Reshape', 'Transpose'):
            target[n.name] = 'cpu'
    print(f"  op_target: {len(target)} ops forced to CPU")
    print(f"    Reshape: {sum(1 for v in target.values() if True)}")
    return target

if __name__ == '__main__':
    from rknn.api import RKNN

    print("Getting op_target map...")
    op_target = get_op_target()
    ...
    print(f"\nLoading ONNX: {CUMFIX_ONNX}")
    ret = rknn.load_onnx(model=CUMFIX_ONNX, inputs=INPUT_NAMES, input_size_list=INPUT_SHAPES)
    assert ret == 0, f"load_onnx failed: {ret}"

    print("\nBuilding INT8 (with op_target)...")
    ret = rknn.build(do_quantization=True, dataset=dataset_txt)
    assert ret == 0, f"build failed: {ret}"

    out_path = f'{RKNN_DIR}/encoder-epoch-99-avg-1-int8-cumfix-optarget.rknn'
